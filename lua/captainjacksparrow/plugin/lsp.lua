return {
  ...
  config = function()
    ...
    vim.api.nvim_create_autocmd('LspAttach', {
      group = vim.api.nvim_create_augroup('jacksparrow-lsp-attach', { clear = true }),
      callback = function(event)
        ...
        if client and client.supports_method(vim.lsp.protocol.Methods.textDocument_documentHighlight) then
          local highlight_augroup = vim.api.nvim_create_augroup('jacksparrow-lsp-highlight', { clear = false })
          ...
          vim.api.nvim_create_autocmd('LspDetach', {
            group = vim.api.nvim_create_augroup('jacksparrow-lsp-detach', { clear = true }),
            callback = function(event2)
              vim.lsp.buf.clear_references()
              vim.api.nvim_clear_autocmds { group = 'jacksparrow-lsp-highlight', buffer = event2.buf }
            end,
          })
        end
